...
The compounding effect works through several mechanisms. First, as the codebase grows, the probability of any new AI-generated code conflicting with existing AI-generated code increases. The AI doesn't have a deep model of your specific system's invariants, so it can easily generate code that violates an assumption made by code it generated three months ago. Second, as the team turns over (and teams always turn over) the new members have even less context than the original vibe coders, because at least the original developers knew what prompts they used. The new people just see a pile of code with no discernible rationale. Third, the patches and fixes that accumulate over time form their own layer of sediment, each one a local fix that may or may not be consistent with the other local fixes around it.
...
**Invest in the boring stuff.** Documentation. Architecture decision records. Meaningful commit messages. Code review where the reviewer actually asks "why?" and the author actually answers. These are the mechanisms that keep institutional knowledge alive, and they're more important than ever in a world where code can be generated faster than it can be understood.

**Set team norms explicitly.** On my team, we have a rule: if you used AI to generate a significant piece of code, you need to be able to explain it in the PR description. Not "AI generated this," actually explain the approach, the tradeoffs, the alternatives you considered. If you can't do that, you're not ready to ship it. This single norm has done more to improve our code quality than any linting rule or CI check.
...
The broader story of this moment in software engineering isn't really about AI at all. It's about the perennial tension between speed and understanding, between shipping and craftsmanship, between what's possible and what's sustainable. Every generation of tools has shifted the balance, and every time, the industry has had to develop new practices to restore it. We'll do it again. We always do. But the cost of the adjustment period, measured in production outages, in burned-out engineers debugging code they don't understand, in products that slowly decay under the weight of their own accidental complexity, that cost is real, and it falls on real people. Let's try to minimize it.
